...
const CartPage = () => {
    const { items, totalAmount, removeItem, updateItem,caleanItem } = useCartStore()
                                ...
                                <div className="justify-between mb-6 rounded-lg bg-white p-6 shadow-md sm:flex sm:justify-start" key={item.id}>
                                    ...
                                    <div className="sm:ml-4 sm:flex sm:w-full sm:justify-between">
                                        <div className="mt-5 sm:mt-0">
                                            ...
                                            <p className="mt-1 text-xs text-gray-700">尺码:{item.size}</p>
                                        </div>
                                        <div className="mt-4 flex justify-between sm:space-y-6 sm:mt-0 sm:block sm:space-x-6">
                                            <div className="flex items-center border-gray-100">
                                                <span className="cursor-pointer rounded-l bg-gray-100 py-1 px-3.5 text-xl duration-100 hover:bg-blue-500 hover:text-blue-50" onClick={() => updateItem(item.id, item.quantity - 1)}> - </span>
                                                <span className="bg-gray-100 px-4 text-xl py-1">{item.quantity}</span>
                                                <span className="cursor-pointer rounded-r bg-gray-100 text-xl py-1 px-3 duration-100 hover:bg-blue-500 hover:text-blue-50" onClick={() => updateItem(item.id, item.quantity + 1)}> + </span>

                                            </div>

                                            <div className="flex items-center justify-between flex-col justify-end ">
                                                <p className="text-sm my-4">单价:$ {item.price}</p>
                                                <button
